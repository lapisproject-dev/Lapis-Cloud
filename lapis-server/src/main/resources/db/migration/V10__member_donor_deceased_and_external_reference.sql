...
-- the already-migrated PROD_HOST/ELB instances reach the same end state. No genuinely new table is
...
-- rolled out on PROD_HOST (>= V1.2.6); their checksums are consumed. An in-place edit of either would
...
--
-- Dual-DROP like V3/V4/V6/V7/V8/V9: PROD_HOST carries the explicitly named chk_member_status (since
-- V3), a genuinely old instance might still carry the Postgres-auto-generated member_status_check.
-- Exactly one of the two DROPs matches per environment. VERIFY WITH `\d member` ON PROD_HOST/ELB BEFORE
-- DEPLOY.
-- ---------------------------------------------------------------------------
