...
private fun testStroke(
    ...
    color = "#1a1a1a",
    strokeWidth = 4.0,
    points = List(pointCount) { WhiteboardPointDto(x = it.toDouble(), y = it.toDouble()) },
...
class ConferenceWhiteboardStateTest :
    FunSpec({
        ...
        test("tryCommit: strokes accumulate and are returned by snapshot in commit order") {
            ...
            state.tryCommit(roomId = roomId, stroke = testStroke(strokeId = "s1")).shouldBeTrue()
            state.tryCommit(roomId = roomId, stroke = testStroke(strokeId = "s2")).shouldBeTrue()
        ...
        test("tryCommit: two different rooms have fully independent state") {
            ...
            state.tryCommit(roomId = roomA, stroke = testStroke(strokeId = "a1")).shouldBeTrue()
            // roomA is now at its own stroke cap (1) -- roomB is unaffected.
            state.tryCommit(roomId = roomB, stroke = testStroke(strokeId = "b1")).shouldBeTrue()
        ...
        test("tryCommit: the STROKE cap rejects the (N+1)th commit even though the point cap is nowhere near hit -- many small strokes") {
            ...
            val roomId = Uuid.random()
            repeat(3) { i -> state.tryCommit(roomId = roomId, stroke = testStroke(strokeId = "s$i", pointCount = 2)).shouldBeTrue() }
            // 4th stroke: stroke count would become 4 > cap of 3 -- rejected, even though total points
            // (8) is nowhere near the 1,000 point cap.
            state.tryCommit(roomId = roomId, stroke = testStroke(strokeId = "s-over", pointCount = 2)).shouldBeFalse()
        ...
        test("tryCommit: the POINT cap rejects a single large-point stroke even though the stroke cap is nowhere near hit") {
            ...
            val roomId = Uuid.random()
            state.tryCommit(roomId = roomId, stroke = testStroke(strokeId = "small", pointCount = 10)).shouldBeTrue()
            // 10 + 45 = 55 > 50 -- rejected, even though stroke count (would become 2) is nowhere
            // near the 1,000 stroke cap.
            state.tryCommit(roomId = roomId, stroke = testStroke(strokeId = "large", pointCount = 45)).shouldBeFalse()
        ...
        test("tryCommit: a rejected commit leaves prior state completely intact (no partial mutation)") {
            ...
            val roomId = Uuid.random()
            state.tryCommit(roomId = roomId, stroke = testStroke(strokeId = "kept")).shouldBeTrue()
            state.tryCommit(roomId = roomId, stroke = testStroke(strokeId = "rejected")).shouldBeFalse()
        ...
        test(
            ...
            val roomId = Uuid.random()
            state.tryCommit(roomId = roomId, stroke = testStroke(strokeId = "s1", pointCount = 3)).shouldBeTrue()
