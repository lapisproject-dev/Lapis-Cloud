...
class DomainModelMergerTest :
    FunSpec({
        ...
        test(
            ...
            // Welle V1.4.3.1 "Veranstaltungen: Kernschleife + Anmeldegebuehren-Zahlung" adds
            // 39-events.kuml.kts's TWO real tables (event, event_registration), WITH its own
            // cross-domain Member stub (event.created_by/event_registration.member_id resolve
            ...
            // distinct table names versus the V1.4.2 baseline above (123 -> 125). 33-payments
            // .kuml.kts also gains its own id-only EventRegistration stub (for
            // paymentCheckoutSession.eventRegistrationId) -- it dedups into 39-events.kuml.kts's
            // now-real event_registration entity (the fuller declaration always wins as canonical,
            ...
            // Welle V1.4.4.3 "Mitgliederlebenszyklus: Ehrungsverwaltung" adds
            // 41-member-honor.kuml.kts's ONE real table (member_honor), WITH its own cross-domain
            // Member stub (member_honor.member_id/recorded_by both resolve through it) -- the stub
            // dedups into the already-real member entity -- so it contributes +2 «Entity»
            // declarations (the stub + 1 real table) and 1 drop, net +1 distinct table name versus
            // the V1.4.5.1 baseline above (127 -> 128).
            // Welle V1.4.4.4 "Familienmitgliedschaften" adds 42-member-family.kuml.kts's TWO real
            // tables (member_family, member_family_link), WITH its own cross-domain Member stub
            // (member_family.created_by/member_family_link.member_id/.linked_by all resolve
            // through it) -- the stub dedups into the already-real member entity -- so it
            // contributes +3 «Entity» declarations (the stub + 2 real tables) and 1 drop, net +2
            ...
            // Welle V1.4.11 "Reisekostenabrechnung für Vorstand und Funktionsträger" adds
            // 45-travel-expense.kuml.kts's THREE real tables (travel_expense_report,
            // travel_expense_line, travel_expense_receipt), WITH TWO cross-domain stubs (Member,
            // JournalEntry) -- both stubs dedup into already-real entities (member/journal_entry)
            // -- so it contributes +5 «Entity» declarations (2 stubs + 3 real tables) and 2 drops,
            // net +3 distinct table names versus the V1.4.10 baseline above (135 -> 138).
            // Welle V1.4.12 "Übungsleiter- und Ehrenamtspauschale" adds
            // 46-volunteer-allowance.kuml.kts's TWO real tables (volunteer_allowance_payment,
            // volunteer_allowance_self_declaration), WITH TWO cross-domain stubs (Member,
            // JournalEntry) -- both stubs dedup into already-real entities (member/journal_entry)
            ...
            // Welle V1.4.3.7 "Helfer-/Schichtplanung für Veranstaltungen" adds
            // 51-event-volunteer.kuml.kts's TWO real tables (event_volunteer_shift,
            // event_volunteer_signup), WITH TWO cross-domain stubs (Member, Event -- both dedup
            // into already-real entities: 00-foundation.kuml.kts's member and
            ...
            // Welle "Digitaler Mitgliedsausweis (PDF)" adds 52-member-card.kuml.kts's TWO real
            // tables (member_number_sequence, member_card_code), WITH ONE cross-domain Member stub
            ...
            fileNames shouldContainExactlyInAnyOrder
                listOf(
                    ...
                    // two new real tables (event, event_registration); the Member cross-domain stub
                    // (39-events.kuml.kts) and the EventRegistration cross-domain stub
                    // (33-payments.kuml.kts) both dedup into already-real entities, no new Table
                    // file for either.
                    "EventTable.kt",
                    ...
                    // Welle V1.4.4.3 "Mitgliederlebenszyklus: Ehrungsverwaltung" -- one new real
                    // table (member_honor); its Member cross-domain stub dedups into the
                    // already-real member entity, no new Table file for it.
                    "MemberHonorTable.kt",
                    // Welle V1.4.4.4 "Familienmitgliedschaften" -- two new real tables
                    // (member_family, member_family_link); their Member cross-domain stub dedups
                    ...
                    // three new real tables (travel_expense_report, travel_expense_line,
                    // travel_expense_receipt); its Member/JournalEntry cross-domain stubs both
                    // dedup into already-real entities, no new Table file for either.
                    "TravelExpenseReportTable.kt",
                    "TravelExpenseLineTable.kt",
                    "TravelExpenseReceiptTable.kt",
                    // Welle V1.4.12 "Übungsleiter- und Ehrenamtspauschale" -- two new real tables
                    // (volunteer_allowance_payment, volunteer_allowance_self_declaration); its
                    // Member/JournalEntry cross-domain stubs both dedup into already-real entities,
                    ...
                    // Welle V1.4.3.7 "Helfer-/Schichtplanung für Veranstaltungen" -- TWO new real
                    // tables (event_volunteer_shift, event_volunteer_signup); its Member/Event
                    // cross-domain stubs both dedup into already-real entities, no new Table file
                    // for either.
                    "EventVolunteerShiftTable.kt",
                    "EventVolunteerSignupTable.kt",
                    // Welle "Digitaler Mitgliedsausweis (PDF)" -- TWO new real tables
                    // (member_number_sequence, member_card_code); its Member cross-domain stub
