...
# .env file. Identical to ../production-elb/render-secrets.sh -- copied verbatim, only the
# directory it runs from differs.
# Run from deploy/production-staging/ (or pass no args, it cd's to its own location).
#
...
# LAPIS_TURN_REALM used to be a prose redaction placeholder (`realm=STAGING_HOST`) hand-edited
...
case "$LAPIS_TURN_REALM" in
  PROD_HOST | ELB_HOST | STAGING_HOST)
    ...
    echo "  (see the NOTE block near the top of ../production/README.adoc)" >&2
    exit 1
    ;;
esac
