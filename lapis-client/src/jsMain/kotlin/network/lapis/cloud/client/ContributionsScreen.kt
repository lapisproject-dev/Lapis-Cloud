...
private fun renderOwnSummary(
    ...
    // contributions table is still correct, only the deferral hint is missing.
    val section =
        panel.dataSection<OwnContributionsData>(
            isEmpty = { false },
            load = {
                val summary = guarded { rpcService<IContributionService>().getMemberContributionSummary(memberId) }
                if (summary == null) {
                    null
                } else {
                    OwnContributionsData(
                        summary = summary,
                        reliefRequests = guarded { rpcService<IContributionReliefService>().listMyReliefRequests() }.orEmpty(),
                    )
                }
            },
            render = { content, data -> renderOwnContributions(content, data, memberId, onReliefChanged) },
        )
    section.reload()
}

private class OwnContributionsData(
    val summary: MemberContributionSummaryDto,
    val reliefRequests: List<ContributionReliefRequestDto>,
)

private fun renderOwnContributions(
    content: SimplePanel,
    data: OwnContributionsData,
    memberId: String,
    onReliefChanged: () -> Unit,
) {
    ...
    val historyLink = content.button(tr("Vollständige Beitragshistorie"), style = ButtonStyle.LINK)
    ...
    if (summary.contributions.isEmpty()) {
        content.p(tr("Keine Beiträge vorhanden."))
        return
    }
    // The period is the row's identity here: primary column, left-aligned even though it is made of dates
    // (guideline K8); the amount is the numeric column.
    content.dataTable(
        columns = ownContributionColumns(blockingRequest),
...
private fun ownContributionColumns(blockingRequest: ContributionReliefRequestDto?): List<DataColumn<ContributionDto>> =
    listOf(
        ...
        DataColumn(
            title = tr("Status"),
            cell = { container, contribution ->
                container.statusBadge(contributionStatusLabel(contribution.status), contributionStatusColor(contribution.status))
                if (blockingRequest != null && contribution.id == blockingRequest.deferralContributionId) {
                    container.typeBadge(tr("Stundung beantragt"), "warning")
                }
            },
        ),
    )
...
private fun renderOrgWideContributions(root: SimplePanel) {
    ...
    // Welle V1.4.25: `dataSection` + `dataTable` (loading/error/empty states, card list below 768 px).
    // `isPostalMailEnabled()` and `listContributions` are bundled into one load: the postal button of
    // every row depends on the former, so the rows must not render before both are known.
    lateinit var section: DataSection
    section =
        root.dataSection<OrgContributionsData>(
            ...
            render = { content, data ->
                content.dataTable(
                    columns = orgContributionColumns(),
                    rows = data.contributions,
                    actions = { actions, contribution ->
                        renderContributionActions(actions, contribution, canMarkPaid, canWaive, data.postalMailEnabled) { section.reload() }
                    },
                )
            },
        )
...
private class OrgContributionsData(
    val postalMailEnabled: Boolean,
    val contributions: List<ContributionDto>,
)

private fun orgContributionColumns(): List<DataColumn<ContributionDto>> =
    listOf(
...
private fun renderContributionActions(
    ...
    val actionsPanel = actionsContainer.vPanel(spacing = 4)
