...
* **FRIEND email verification is present but unenforced, for the same reason.** `MemberStatus.FRIEND`
  (V0.11.0) is a self-registerable status (`/register-friend`, no board approval, no identity check)
  scoped strictly to video-conference access on a per-room opt-in basis — it is not membership in
  the organization, carries no Beitragspflicht/governance/accounting/LTR rights, and no
  `PUBLIC_MEMBERS` document access. The `friend_email_verification_token` mechanics exist end to
  end, but enforcement stays behind `LAPIS_FRIEND_REQUIRE_EMAIL_VERIFICATION` (default `false`):
  same root cause as the bullet above, no real SMTP transport exists anywhere in this codebase yet
