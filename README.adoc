...
As of *V0.8.4* (tag `v0.8.0`), eight major waves are complete (see `CHANGELOG.md` for full
detail):
...
participation in the LTR economy yet, DNS-rebinding TOCTOU gap in the federation SSRF guard) are
tracked in `CHANGELOG.md`. See link:docs/architecture/domain-model.adoc[domain model
...
`CHANGELOG.md` `[0.6.0]` for the original scope-cut reasoning and the latest `[Unreleased]` entry
for the closure.
...
* **UI coverage beyond the core domains.** Governance, accounting, compliance, and the entire LTR
  economy have no UI yet — reachable only via Kilua RPC / the HTTP routes. Needs its own future
  prioritization decision, not yet scheduled as a numbered wave.
...
* **DNS-rebinding TOCTOU gap in the federation SSRF guard** (`requireSafeFederationUrl`, disclosed
  since V0.8.1): the target host's DNS is resolved and validated, but the actual HTTP connection
  that follows performs its own, independent resolution — a malicious DNS server could rebind
  between the two. Closing this fully requires pinning the validated IP for the actual connection.
* **`GovernanceService.addCommitteeMember` never validates the seated member's status.** A
  Committee-membership-based voting-eligibility path (`castVoteBallot`/`castElectionBallot`/
  `castResistanceBallot`) therefore still depends on the seating call itself being used correctly —
  those three cast methods are independently gated to `AKTIV`-only as defense in depth, but the
  root cause (an `ANTRAG` member could in principle be seated onto a Committee) is unfixed.
* **`RegistrationService.rejectApplication` does not revoke the caller's session** the way
  `leaveMembership` does — a rejected (`ABGELEHNT`) applicant's existing session can outlive their
  rejection, though every write path that matters is independently `AKTIV`-gated.
