...
As of tag `v0.9.0`, eight major waves are complete (see `CHANGELOG.md` for full detail). `v0.9.0`
...
**Since `v0.9.0` (on `master`, not yet tagged):** a V1.0-readiness review with this project's two
real pilot organizations found that 13 backend domains had zero client UI, reachable only via
Kilua RPC / raw HTTP — a treasurer or board member without developer access could not use any of
them. The pilots picked Governance and Accounting as the two highest-priority gaps to close first.
Both are done: a **Governance UI** (committees/working-group membership, meeting management with a
live-recalculated quorum display and a printable protocol/Beschlussbuch draft, and the full
motion/amendment/voting workflow) and an **Accounting UI** (the SKR42 chart of accounts and
journal with a draft-then-post workflow, GuV/Bilanz/Jahresabschluss, the four-sphere
...
implementation. See `CHANGELOG.md`'s `[Unreleased]` section for full detail on both waves.
