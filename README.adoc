...
== Website Integration (V1.4.1a)

A small (< 8 KB, no dependencies) embeddable script lets a partner website — a party's or
association's own site, on a *different* origin — offer a member-login button and a
"become a member" link without ever sharing a session cookie across that origin boundary. Login
runs in a popup window on this server's own origin; only a `postMessage`-delivered
...
Deliberately out of scope, reserved for a future wave (tentatively V1.4.1b) building on this
wave's `EmbedConfig`/`EmbedCors`/`EmbedAssets` infrastructure: an anonymous donation widget, any
donation-checkout endpoint under `/api/embed/v1/`, and an automatic "already signed in" indicator
on page load without a click (deliberately absent — that would be a cross-site tracking channel).
