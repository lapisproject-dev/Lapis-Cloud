...
default and must be enabled on at most one instance, see `deploy/production/README.adoc`, "Price-Oracle
Snapshot Poller"); a BOARD/ADMIN screen for creating and managing events; and a dedicated staging
deployment (`deploy/production-staging/`, fictitious seed data only). **V1.6.1 optional AI assistance**
...
`deploy/production/.env.example`/`deploy/production-elb/.env.example` for the full block.
...
provider first (`deploy/production/README.adoc`, section "AI assistance").
