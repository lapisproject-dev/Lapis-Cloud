...
== Website Integration (V1.4.1a/V1.4.1b)
...
"become a member" link, and an anonymous donation form, without ever sharing a session cookie
across that origin boundary. Login runs in a popup window on this server's own origin; only a `postMessage`-delivered
...
== Bank Statement Import (V1.4.5.1)
...
at all) — only the reference rule ever books automatically. See
`docs/architecture/bank-statement-import.adoc` for the full design (data shape, the GF(2⁵)-checksum
payment-reference grammar, the matching pipeline, and the two-phase booking transaction that keeps
`ContributionPostingBridge`'s single-row-lock-per-transaction contract).

=== What doesn't work yet (this wave)

Deliberately out of scope, see `CHANGELOG.md`: the KVision operator screen (`/bank-import`) and the
full seven-language i18n catalog update — the server-side surface (migration, kUML model, parsers,
matcher, booking path, `IBankStatementService` RPC, and the
`POST /api/bank-statements/import` upload route) is complete and tested, only the UI is deferred.
Also out of scope: DATEV/lexoffice/sevDesk export (see the next section — the DATEV half is closed
as of V1.4.5.2), camt.052/053/054 XML, FinTS/HBCI, splitting one bank line across multiple
contributions, automatic *donation* posting (§25 PartG: a donor category cannot be derived from a
bank line alone), and header signatures for the `VR_BANK`/`DKB`/`POSTBANK`/`COMDIRECT` CSV
dialects (no real export sample was available to verify one against).
