...
Twelve further tags have shipped since `v0.10.0`, two of them (`v0.16.0`/`v0.17.0`) cut back to back
...
Ten fixed events (`committee.created`/`.updated`, `meeting.created`/`.held`, `resolution.adopted`,
`motion.scheduled`, `member.created`, `contribution.paid`, `donation.received`, `webhook.test`) fan
out as signed HTTPS POSTs, one webhook endpoint per existing `Authorization: Bearer lapis_<key>` API
key from the Public REST API above — there is no separate webhook-only credential namespace.
