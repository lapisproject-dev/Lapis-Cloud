...
this codebase. **That condition is now met** — V0.8.2 gives every guest a real `Member(status =
GAST)` row — so whether to revisit the rating-basket cut is now a live, open product decision
rather than a deferred one. Not yet revisited as of this release. See `CHANGELOG.md` `[0.6.0]`
"Known limitations" for the original reasoning.
...
* **`PeerTransferService.transferLtr`/`GovernanceService.castVoteBallot` don't gate on active
  membership** (known, disclosed gap, tracked as a follow-up wave): an `ANTRAG` applicant could in
  principle stake/transfer LTR before board approval.
