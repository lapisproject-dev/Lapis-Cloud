...
Also out of scope: DATEV/lexoffice/sevDesk export, camt.052/053/054 XML, FinTS/HBCI, splitting one
bank line across multiple contributions, automatic *donation* posting (§25 PartG: a donor category
cannot be derived from a bank line alone), and header signatures for the `VR_BANK`/`DKB`/
`POSTBANK`/`COMDIRECT` CSV dialects (no real export sample was available to verify one against).
